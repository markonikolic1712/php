...
for($i = 0; $i<count($players); $i++){
	$player = $players[$i];
	$strana = $player['position'];
	$visina = $player['lane'];
	$rolePosition = $rolePositions[$strana][$visina];
	$slicicaPozicija = $heroSpritePositions[$player['hero']];

	$backgroundPosition = "background-position: {$slicicaPozicija[0]}px {$slicicaPozicija[1]}px";
